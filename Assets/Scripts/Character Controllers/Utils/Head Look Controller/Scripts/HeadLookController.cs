...
public class HeadLookController : MonoBehaviour {
	...
	void LateUpdate () {
		if (Time.deltaTime == 0 || !target)
			return;
		
		//if (animator.GetBool("IsAlert") == false)
			//return;
		...
		foreach (BendingSegment segment in segments) {
			...
			Vector3 lookDirWorld = (target.transform.position - segment.lastTransform.position).normalized;
            ...
            // Lerp angles * segment.responsiveness
			...
			segment.dirUp = Vector3.Slerp(segment.dirUp, upDirGoal, .25f);//Time.deltaTime*
